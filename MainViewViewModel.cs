...
namespace RevitAPITrainingElemBetweenPoint
{
    public class MainViewViewModel
    {
        ...
        private void OnSaveCommand()
        {
            ...
            if (ElementCount <= 0)
            {
                MessageBox.Show("Кол-во элементов меньше 0");
                return;
            }
            ...
            {
                RaiseCloseRequest();
                Points = SelectionUtils.GetPoints(_commandData, "Выберите точки для вставки элементов", ObjectSnapTypes.Endpoints, 1);
                PointEl.Add(Points[0]);
            }
            else
            {
                RaiseCloseRequest();
                Points = SelectionUtils.GetPoints(_commandData, "Выберите точки для вставки элементов", ObjectSnapTypes.Endpoints, 2);
